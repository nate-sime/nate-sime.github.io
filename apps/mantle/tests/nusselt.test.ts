 ...
 * naming d and κ while the conversion used something else is worse than no note
...
describe("dimensional time", () => {
  it("converts through the reference it prints", () => {
    ...
    // reference alone.
    expect(timeUnitYears).toBeCloseTo(lengthScale ** 2 / REFERENCE.kappa / YEAR, -6);
    const note = referenceNote();
    expect(note).toContain(REFERENCE.kappa.toExponential(0));
    expect(note).toContain(`${(lengthScale / 1e3).toFixed(0)} km`);
    expect(note).toContain(timeUnitYears.toExponential(2));
  });
   ...
   * it convects. If this fell to the low Gyr the reference would have silently
   * become something other than whole-mantle.
   */
  it("puts one diffusion time in the hundreds of billions of years", () => {
    expect(timeUnitYears).toBeGreaterThan(1e11);
    expect(timeUnitYears).toBeLessThan(1e12);
  });
  ...
  it("picks a unit that keeps three significant figures", () => {
    ...
    expect(dimensionalTime(1)).toMatch(/Gyr$/);
